"""OPPSS GENERATOR v15.1 GUI STANDALONE - SQLite baza, CSV export, pretraga, izveštaji.
...
class Database:
    ...
    def statistika(self):
        ...
        # Po opštini
        cursor.execute("SELECT opstina, COUNT(*), SUM(iznos_prometa) FROM ljudi WHERE godina = ? GROUP BY opština ORDER BY SUM(iznos_prometa) DESC", (self.godina,))
...
class DatumEntry(ttk.Entry):
    ...
    def _fmt(self, ev):
        # Ne radiš ništa za navigacione tastere - kursor ostaje tamo gdje je
        if ev.keysym in ("BackSpace", "Delete", "Left", "Right", "Home", "End", "Tab", "Return"):
            return
        
        # Ne radiš ništa ako nije unet novi karakter (npr. samo modifier)
        if len(ev.keysym) > 1 and ev.keysym not in ("space",):
            return
        
        # Sačaj poziciju kursora (broj cifara pre kursora)
        ...
        # Pronađi novu poziciju kursora - isti broj cifara s leve strane
        digit_count = 0
        for i, ch in enumerate(formatted):
            if ch.isdigit():
                digit_count += 1
                if digit_count >= digits_before_cursor:
                    # Postavi kursor iza ove cifre
                    self.icursor(min(i + 1, len(formatted)))
                    return
        
        # Ako nema cifara (prazno polje), ostavi kursor na početku
        self.icursor(0)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OPPSS Generator v15.1 GUI STANDALONE - ePorezi prijava")
        ...
        # BUG 3 FIX: Dvoklik na red u tabeli za izmenu
        ...
        ttk.Label(status_bar, text="F1 = Pomoć | Ctrl+N = Novi | Ctrl+D = Obriši | Ctrl+F = Pretraga | Ctrl+G = XML | Ctrl+P = Podnosioc | Dvoklik = Izmeni",
    ...
    def prikazi_about(self):
        messagebox.showinfo("O aplikaciji",
                            "OPPSS Generator v15.1 GUI STANDALONE\n\n"
    ...
    def forma_osobe(self, podrazumevano=None, indeks_izmene=None):
        ...
        BUG 3 FIX: Dugme menja tekst u 'Sačuvaj izmene' kada se menja postojeći unos."""
        ...
        # BUG 2 FIX: Info o duplikatu - prikazuje upozorenje, ali ne automatski popunjava
        ...
        def na_kucanje(ev=None):
            """Provera duplikata prilikom kucanja - prikazuje upozorenje."""
            try:
                ...
                # BUG 2 FIX: Prikaži upozorenje ako postoji isti identifikator (ali ne popunjavaj)
        ...
        if podrazumevano:
            ...
            for key, w in entries.items():
                if key in ("datum_unos", "datum_do"):
                    w.delete(0, "end")
                    if podrazumevano.get(key.replace("unos", "datum")):
                        try:
        ...
        def sacuvaj():
            """Čuva unos u bazu.
            BUG 2 FIX: Provera duplikata uz upozorenje ali bez blokiranja."""
            ...
            # BUG 2 FIX: Provera duplikata identifikatora uz upozorenje
            ljudi = self.db.ucitaj_ljude()
        ...
        # BUG 3 FIX: Dugme menja tekst u zavisnosti od akcije
