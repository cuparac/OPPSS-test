"""OPPSS GENERATOR v15.2 GUI STANDALONE - SQLite baza, CSV export, pretraga, izveštaji.
...
- Brisanje svih unosa odjednom (sa potvrdom)
- Backup baze podataka
- Import iz CSV fajla
- Izvještaj za štampu (HTML/PDF)
- Sortiranje tabele po kolonama (klikom na zaglavlje)
- Kontekstni meni (desni klik na red ili prazan prostor)

v15.1 ispravke:
- Bug 1: DatumEntry kursor se može pozicionirati bilo gde
- Bug 2: Provera duplikata identifikatora uz upozorenje (ali dozvoljava unos)
- Bug 3: Dvoklik na red u tabeli za izmenu + dugme menja tekst u 'Sačuvaj izmene'"""

import json, os, sys, datetime, calendar as _cal, platform, sqlite3, csv, shutil, webbrowser
...
class Database:
    ...
    def obrisi_sve(self):
        """Briše sve osobe za izabranu godinu."""
        cursor = self.conn.cursor()
        cursor.execute("DELETE FROM ljudi WHERE godina = ?", (self.godina,))
        self.conn.commit()
    ...
    def statistika(self):
        ...
        return {
            'ukupno_unosa': ukupno[0],
            'ukupno_iznos': ukupno[1],
    ...
    def import_csv(self, fajl_putanja):
        """Import podataka iz CSV fajla. Vraća (uspeh, poruka, broj_unetih)."""
        if not os.path.exists(fajl_putanja):
            return False, "Fajl ne postoji", 0
        ...
        try:
            ...
            return False, f"Greška pri čitanju CSV-a: {e}", 0
        
        if not rows:
            return False, "CSV fajl je prazan", 0
        
        tipovi_mapa = {"1": "1", "2": "2", "Poljoprivreda": "1", "Sirovine": "2", 
                       "Poljoprivredni proizvodi/usluge": "1", "Sekundarne sirovine": "2"}
        id_mapa = {"JMBG": "1", "PIB": "0", "EBS": "5"}
        ...
        for i, row in enumerate(rows, 1):
            try:
                ...
                vrsta_pr = (row.get('Vrsta prometa', '') or row.get('vrsta_prometa', '') or 
                           row.get('VrstaPrometa', '1'))
                vrsta_pr = tipovi_mapa.get(vrsta_pr, vrsta_pr) or '1'
                
                vrsta_id = (row.get('Vrsta ID', '') or row.get('vrsta_identifikatora', '') or
                           row.get('VrstaIdentifikatora', '0'))
                vrsta_id = id_mapa.get(vrsta_id, vrsta_id) or '0'
                
                identifikator = (row.get('Identifikator', '') or row.get('identifikator', '') or
                                row.get('IdentifikatorIzvrsiocaPrometa', '')).strip()
                
                if not identifikator:
                    greske.append(f"Red {i}: Nedostaje identifikator")
                    continue
                
                ime_naziv = (row.get('Ime/Naziv', '') or row.get('ime_naziv', '') or
                            row.get('ImePrezimeNaziv', '')).strip()
                opstina = (row.get('Opština', '') or row.get('opstina', '') or
                          row.get('OpstinaPrebivalista', '')).strip()
                adresa = (row.get('Adresa', '') or row.get('adresa', '')).strip()
                email_osobe = (row.get('Email', '') or row.get('email_osobe', '') or
                              row.get('ElektronskaAdresa', '')).strip()
                telefon = (row.get('Telefon', '') or row.get('telefon', '')).strip()
                broj_gazdinstva = (row.get('Br. gazdinstva', '') or row.get('broj_gazdinstva', '') or
                                  row.get('BrojPoljoprivrednogGazdinstva', '')).strip()
                naziv_gazdinstva = (row.get('Naziv gazdinstva', '') or row.get('naziv_gazdinstva', '') or
                                   row.get('NazivPoljoprivrednogGazdinstva', '')).strip()
                
                datum_str = (row.get('Datum OD', '') or row.get('datum', '') or
                            row.get('DatumOd', ''))
                datum_do_str = (row.get('Datum DO', '') or row.get('datum_do', '') or
                               row.get('DatumDo', datum_str))
                
                # Pokušaj konverziju datuma (podržavi Y-m-d i d/m/Y)
                datum = None
                for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%d.%m.%Y", "%Y/%m/%d"):
                    try:
                        datum = datetime.datetime.strptime(datum_str.strip(), fmt).strftime("%Y-%m-%d")
                        break
                    except ValueError:
                        continue
                
                if not datum:
                    greske.append(f"Red {i}: Neispravan datum '{datum_str}'")
                    continue
                
                datum_do = None
                for fmt in ("%Y-%m-%d", "%d/%m/%Y", "%d.%m.%Y", "%Y/%m/%d"):
                    try:
                        datum_do = datetime.datetime.strptime(datum_do_str.strip(), fmt).strftime("%Y-%m-%d")
                        break
                    except ValueError:
                        continue
                ...
                iznos_str = (row.get('Iznos', '') or row.get('iznos_prometa', '') or
                            row.get('IznosPrometa', '0'))
                try:
                    iznos = int(str(iznos_str).replace('.', '').replace(',', '').strip())
                except ValueError:
                    greske.append(f"Red {i}: Neispravan iznos '{iznos_str}'")
                    continue
                
                if iznos <= 0:
                    greske.append(f"Red {i}: Iznos mora biti pozitivan")
                    continue
                
                podaci = {
                    "vrsta_prometa": vrsta_pr,
                    "vrsta_identifikatora": vrsta_id,
                ...
                self.dodaj_osobu(podaci)
                uneto += 1
                
            except Exception as e:
                greske.append(f"Red {i}: {e}")
        
        poruka = f"Uveženo {uneto} od {len(rows)} unosa."
        if greske:
            poruka += f"\n\nGreške ({len(greske)}):\n" + "\n".join(greske[:10])
            if len(greske) > 10:
                poruka += f"\n... i još {len(greske) - 10} grešaka"
        
        return uneto > 0, poruka, uneto
...
def generisi_html_izvestaj(app, db, godina):
    """Generiše HTML izveštaj za štampu/PDF."""
    podnosioc = db.ucitaj_podnosioca()
    ljudi = db.ucitaj_ljude()
    stat = db.statistika()
    
    tipovi_prometa = {"1": "Poljoprivredni proizvodi/usluge", "2": "Sekundarne sirovine"}
    tipovi_id = {"0": "PIB", "1": "JMBG", "5": "EBS"}
    
    datum_sada = datetime.datetime.now().strftime("%d.%m.%Y. u %H:%M")
    
    html = f"""<!DOCTYPE html>
<html lang="sr">
<head>
    <meta charset="UTF-8">
    <title>OPPS Izveštaj {godina}</title>
    <style>
        body {{ font-family: 'Segoe UI', Arial, sans-serif; margin: 20px; color: #333; }}
        h1 {{ color: #2c3e50; border-bottom: 3px solid #3498db; padding-bottom: 10px; }}
        h2 {{ color: #2980b9; margin-top: 30px; }}
        .header {{ display: flex; justify-content: space-between; margin-bottom: 20px; }}
        .info-box {{ background: #ecf0f1; padding: 15px; border-radius: 5px; margin-bottom: 20px; }}
        ...
        .footer {{ margin-top: 40px; padding-top: 15px; border-top: 2px solid #eee; font-size: 12px; color: #777; }}
        @media print {{
            body {{ margin: 10px; }}
...
<body>
    <h1>📋 OPPS Izveštaj – {godina}. godina</h1>
    
    <div class="header">
        <div class="info-box">
            <p><span class="label">Podnosioc:</span> {podnosioc.get('pib_jmbg', 'Nije unet')}</p>
            <p><span class="label">JMBG:</span> {podnosioc.get('jmbg', 'Nije unet')}</p>
            <p><span class="label">Email:</span> {podnosioc.get('email', 'Nije unet')}</p>
            <p><span class="label">Telefon:</span> {podnosioc.get('telefon', 'Nije unet')}</p>
        </div>
        <div style="text-align: right; color: #777;">
            <p>Datum izveštaja: {datum_sada}</p>
            <p>Broj unosa: <strong>{len(ljudi)}</strong></p>
        </div>
    </div>
"""
...
"""
        ...
        for i, o in enumerate(ljudi, 1):
            ...
            html += f"""            <tr>
                <td>{i}</td>
                <td>{tipovi_prometa.get(o.get('vrsta_prometa', ''), '?')}</td>
                <td>{tipovi_id.get(o.get('vrsta_identifikatora', ''), '?')}</td>
                <td>{o.get('identifikator', '')}</td>
                <td>{o.get('ime_naziv', '')}</td>
                <td>{o.get('opstina', '')}</td>
                <td>{o.get('adresa', '')}</td>
                <td>{datum_od}</td>
                <td>{datum_do}</td>
                <td class="right">{format(iznos, ',').replace(',', '.')}</td>
            </tr>
"""
    ...
    <div class="summary">
        <h3>📊 Ukupan pregled</h3>
        <p><strong>Ukupan broj unosa:</strong> {len(ljudi)}</p>
        <p><strong>Ukupan iznos:</strong> {format(ukupno, ',').replace(',', '.')} RSD</p>
    </div>
"""
    ...
    <table>
        <thead>
            <tr><th>Opština</th><th class="right">Broj unosa</th><th class="right">Ukupan iznos</th></tr>
        </thead>
        <tbody>
"""
        for v in stat['po_opstini']:
            opstina = v.get('opstina', '')
            br_unosa = v.get('COUNT(*)', 0)
            iznos = v.get('SUM(iznos_prometa)', 0)
            html += f"            <tr><td>{opstina}</td><td class='right'>{br_unosa}</td><td class='right'>{format(iznos, ',').replace(',', '.')} RSD</td></tr>\n"
...
"""
    ...
    html += f"""
    <div class="footer">
        <p>Generisano pomoću OPPS Generator v15.2 | {godina}. godina</p>
        <p class="no-print"><button onclick="window.print()">🖨️ Štampaj</button></p>
    </div>
</body>
</html>"""
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OPPSS Generator v15.2 GUI STANDALONE - ePorezi prijava")
    ...
    def prikazi_kontekstni_meni(self, event):
    ...
    def prikazi_about(self):
        messagebox.showinfo("O aplikaciji",
                            "OPPSS Generator v15.2 GUI STANDALONE\n\n"
